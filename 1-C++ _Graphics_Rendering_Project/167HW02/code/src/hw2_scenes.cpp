...
// for 2.2 bonus
    TriangleMesh mesh5 {
            // vertices
            {{-3.2,-2.8,-4.0},{3.2,-2.8,-4.0},{1.2,2.8,-4.0},{5.4,-1.2,-6.0},{1.8,3.0,-6.0},{-0.6,-2.4,-6.0},
                    {4.8,-0.8,-8.0},{3.2,0.8,-8.0},{1.6,-4.8,-8.0},{0.8,-0.8,-8.0},{-4.0,-4.8,-8.0},{1.6,-4.8,-8.0}},
            // faces
            {{0,1,2},{9, 10, 11},{6,7,8},{3,4,5}},
            // face_colors
            {{0.0,0.9,0.2},{0.35, 0.75, 0.35},{1,0,0},{0,0,1}},

    };

std::vector<TriangleMesh> meshes = {mesh0, mesh1, mesh2, mesh3, mesh5};
...
Matrix4x4 parse_transformation(const json &node) {
    ...
    for (auto it = transform_it->begin(); it != transform_it->end(); it++) {
        if (auto scale_it = it->find("scale"); scale_it != it->end()) {
            ...
            m2(0,0)= axis.x*axis.x+(1-axis.x*axis.x)* cos(radians);
            m2(1,0)= axis.x*axis.y*(1- cos(radians))+axis.z* sin(radians);
            m2(2,0)= axis.x*axis.z*(1- cos(radians))-axis.y* sin(radians);
            m2(0,1)= axis.y*axis.x*(1- cos(radians))-axis.z* sin(radians);
            m2(1,1)= axis.y*axis.y+(1-axis.y*axis.y)* cos(radians);
            m2(2,1)= axis.y*axis.z*(1- cos(radians))+axis.x* sin(radians);
            m2(0,2)= axis.z*axis.x*(1- cos(radians))+axis.y* sin(radians);
            m2(1,2)= axis.z*axis.y*(1- cos(radians))-axis.x* sin(radians);
            m2(2,2)= axis.z*axis.z+(1-axis.z*axis.z)* cos(radians);
            F=m2*F;
            ...
            up_ = normalize(cross(right,direction));
            m4(0,0)=right.x;
            m4(1,0)=right.y;
            m4(2,0)=right.z;
            m4(0,1)=up_.x;
